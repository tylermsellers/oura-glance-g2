...
function injectClockText(data, clock) {
...
function injectClockSplit(header, clock) {
...
export function useGlasses(config) {
    ...
    const sendImages = useCallback((tiles) => {
        if (imgBusyRef.current || !hubRef.current)
            return;
    ...
    const maybeHandleHomeShutdown = useCallback(async (action) => {
        if (action.type !== 'GO_BACK')
            return false;
        if (configRef.current.shutdownOnHomeBack === false)
            return false;
        const nav = navRef.current;
        const getMode = configRef.current.getPageMode ?? (() => 'text');
        if (getMode(nav.screen) !== 'home')
            return false;
        const hub = hubRef.current;
        if (!hub)
            return false;
    ...
    const handleAction = useCallback((action) => {
        void (async () => {
            if (await maybeHandleHomeShutdown(action))
                return;
            const snapshot = configRef.current.getSnapshot();
            ...
            if (!disposed) {
                ...
                pollTimer = setInterval(() => {
                    ...
                    if (snapshot !== lastSnapshotRef.current || clockChanged) {
                        ...
                        if (configRef.current.headerClock)
                            lastClockRef.current = currentClock();
                        flushDisplay();
                    }
                }, 100);
            }
