import { GlassesSdk } from './sdk-wrapper';
...
export class EvenHubBridge {
     ...
     */
    async showSplitPage(header, leftOrPanes, rightOrLayout, layout) {
        if (!this._pageReady)
            return;
        let panes;
        let resolvedLayout;
        if (Array.isArray(leftOrPanes)) {
            panes = leftOrPanes;
            resolvedLayout = rightOrLayout;
        }
        else {
            panes = [leftOrPanes, rightOrLayout];
    ...
    async updateSplitPage(header, leftOrPanes, rightOrLayout, layout) {
        if (!this._pageReady)
            return;
        let panes;
        let resolvedLayout;
        ...
        if (this.rawBridge) {
            ...
            notifyTextUpdate();
            const upgrades = [
                this.rawBridge.textContainerUpgrade(new TextContainerUpgrade({
                    containerID: 6, containerName: 'split-header',
                    contentOffset: 0, contentLength: 2000, content: header,
                })),
            ];
            for (let i = 0; i < panes.length; i++) {
                upgrades.push(this.rawBridge.textContainerUpgrade(new TextContainerUpgrade({
                    containerID: 7 + i, containerName: `split-pane-${i}`,
    ...
    async sendImage(containerID, containerName, pngBytes) {
        if (!this.rawBridge || !this._pageReady || this._currentMode === 'text' || this._currentMode === 'columns' || this._currentMode === 'split' || pngBytes.length === 0)
            return;
    ...
    // ── Shutdown ──
    async showShutdownContainer(exitMode = 1) {
        if (!this.rawBridge || !this._pageReady)
            return false;
        try {
            ...
            return await this.rawBridge.callEvenApp('shutDownPageContainer', { exitMode });
        }
        catch {
            return false;
        }
    ...
    async setLocalStorage(key, value) {
        if (!this.rawBridge)
            return false;
        try {
            return await this.rawBridge.setLocalStorage(key, value);
        }
        catch {
            return false;
        }
    ...
    async getLocalStorage(key) {
        if (!this.rawBridge)
            return '';
        try {
            return await this.rawBridge.getLocalStorage(key);
        }
        catch {
            return '';
        }
