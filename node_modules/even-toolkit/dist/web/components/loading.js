...
function Loading({ size = 24, className }) {
    ...
    const cellSize = 2;
    const viewSize = 8;
    return (_jsx("div", { className: cn('inline-flex items-center justify-center', className), style: { width: Math.max(size, 8), height: Math.max(size, 8) }, children: _jsx("svg", { width: Math.max(size, 8), height: Math.max(size, 8), viewBox: `0 0 ${viewSize} ${viewSize}`, shapeRendering: "crispEdges", children: fillOrder.map(([r, c], i) => {
                const on = filling ? i <= t : i > t;
                return (_jsx("rect", { x: c * cellSize, y: r * cellSize, width: cellSize, height: cellSize, fill: "currentColor", opacity: on ? 1 : 0, style: { transition: 'opacity 40ms ease' } }, i));
            }) }) }));
}
export { Loading };
